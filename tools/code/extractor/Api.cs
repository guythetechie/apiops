...
{
    private readonly ApiManagementServiceResource serviceResource;
    private readonly ServiceDirectory serviceDirectory;
    private readonly ILogger logger;
    private readonly Option<Seq<ApiName>> apiNamesToExport;

    public ApiService(ApiManagementServiceResource serviceResource,
                      ServiceDirectory serviceDirectory,
                      ILogger<ApiService> logger,
                      Option<Seq<ApiName>> apiNamesToExport)
    {
        this.serviceResource = serviceResource;
        this.serviceDirectory = serviceDirectory;
        this.logger = logger;
        this.apiNamesToExport = apiNamesToExport;
    }

    public async ValueTask ExportAll(CancellationToken cancellationToken)
    {
        await serviceResource.GetApis()
                             .GetAllAsync(cancellationToken: cancellationToken)
                             .Where(ShouldExport)
                             .ForEachParallel(async resource => await ExportApi(resource, cancellationToken), cancellationToken);
    }

    private bool ShouldExport(ApiResource resource)
    {
        return apiNamesToExport.Map(apiNames => ShouldExport(resource, apiNames))
                               .IfNone(true);
    }

    private static bool ShouldExport(ApiResource resource, Seq<ApiName> apiNamesToExport)
    {
        var apiName = resource.GetApiName();
        return apiNamesToExport.Any(apiNameToExport => ApiNamesMatch(apiName, apiNameToExport));
    }

    private static bool ApiNamesMatch(ApiName firstApiName, ApiName secondApiName)
    {
        return firstApiName.Value.Equals(secondApiName.Value, StringComparison.OrdinalIgnoreCase)
               || GetParentApiName(firstApiName).Value.Equals(GetParentApiName(secondApiName).Value, StringComparison.OrdinalIgnoreCase);
    ...
    /// </summary>
    private static ApiName GetParentApiName(ApiName revisionApiName)
    {
        return new(revisionApiName.Value.Split(';').First());
    }

    private async ValueTask ExportApi(ApiResource resource, CancellationToken cancellationToken)
    {
        await ExportApiInformationFile(resource, cancellationToken);
    }

    private async ValueTask ExportApiInformationFile(ApiResource resource, CancellationToken cancellationToken)
    {
        var apiDirectory = resource.GetApiDirectory(serviceDirectory);
        var informationFile = new ApiInformationFile(apiDirectory);

        var content = resource.Data.ToCreateOrUpdateContent();
        var json = content.Serialize();

        logger.LogInformation("Writing API information file {FilePath}...", informationFile.Path);
        await informationFile.OverwriteWithJson(json, cancellationToken);
    }

    //private static async ValueTask ExportSpecification(ApiResource resource, ApiModel apiModel, ApiDirectory apiDirectory, ApiUri apiUri, DefaultApiSpecification defaultSpecification, GetRestResource getRestResource, DownloadResource downloadResource, ILogger logger, CancellationToken cancellationToken)
    //{
    //    await (resource.Data.ApiType switch
    //    {
    //        var apiType when apiType == ApiType.GraphQL => ExportGraphQlSpecification(apiDirectory, apiUri, getRestResource, logger, cancellationToken),
    //        var apiType when apiType == ApiType.WebSocket => ValueTask.CompletedTask,
    //        var apiType when apiType == ApiType.Soap => ExportWsdlSpecification(apiDirectory, apiUri, getRestResource, downloadResource, logger, cancellationToken),
    ...
    //private async ValueTask<Stream> DownloadSpecificationFile(ApiResource apiResource, string format, GetRestResource getRestResource, DownloadResource downloadResource, CancellationToken cancellationToken)
    //{
    //    var exportUri = apiResource.Id.ur
...
{
    public static ApiName GetApiName(this ApiResource resource)
    {
        return new ApiName(resource.Id.Name);
    }

    public static ApiDirectory GetApiDirectory(this ApiResource resource, ServiceDirectory serviceDirectory)
    {
        var name = resource.GetApiName();
        var apisDirectory = new ApisDirectory(serviceDirectory);
        return new ApiDirectory(name, apisDirectory);
    }
}
